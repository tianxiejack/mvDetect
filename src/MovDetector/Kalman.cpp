...
void CKalman::KalmanInitParam(int x_centre, int y_centre, double DeltaT)
{
	...
	for ( y = 0; y < DP; y++ ){
		process_noise_cov[y*DP+y] = 1.0;  /* 协方差都为1.0，且相互独立 */
	}
	/* 观测噪声协方差矩阵 */
	for ( y = 0; y < MP; y++ ){
		for ( x = 0; x < MP; x++ ){
	...
	for ( y = 0; y < MP; y++ ){
		measurement_noise_cov[y*MP+y] = 1.0;  /* 协方差都为1.0，且相互独立 */
	}
	/* 状态测量误差协方差 */
	for ( y = 0; y < DP; y++ ){
		for ( x = 0; x < DP; x++ ){
	...
	for ( y = 0; y < DP; y++ ){
		error_cov_post[y*DP+y] = 10.0;  /* 对角初始协方差都为10，且相互独立 */
	}
	/* 状态转移阵 */
	for ( y = 0; y < DP; y++ ){
		for ( x = 0; x < DP; x++ ){
	...
	for ( y = 0; y < DP; y++ ){
		transition_matrix[y*DP+y] = 1.0;  /* 对角为1 */
	}
...
void CKalman::KalmanInitParam(int x_centre, int y_centre, double DeltaT)
{
	...
	for ( y = 0; y < DP; y++ ){
		error_cov_post[y*DP+y] = 1.0;  /* 对角初始协方差都为10，且相互独立 */
	}
	/* 状态转移阵 */
	for ( y = 0; y < DP; y++ ){
		for ( x = 0; x < DP; x++ ){
